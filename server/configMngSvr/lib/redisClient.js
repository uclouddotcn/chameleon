...
/**
 *  Redis client
 * @class RedisClient, for product config storage
...
RedisClient.prototype.set = function (key, value, callback) {
    ...
    self.activeClient.set(key, value, function (err, res) {
        if (err) {
...
RedisClient.prototype.hset = function (key, field, value, callback) {
    var self=this;
    if (!self.activeClient) {
        if (callback) {
            callback(new Error('all redis client gone'));
    ...
    self.activeClient.hset(key, field, value, function (err, res) {
        if (err) {
            self.logger.error({err: err}, 'fail to set clients');
        }
        if (callback) {
...
RedisClient.prototype.hget = function(key, field, callback) {
    var self = this;
    if (!self.activeClient) {
        if (callback) {
            callback(new Error('all redis client gone'));
    ...
    self.activeClient.hget(key, field, function (err, res) {
        if (err) {
            self.logger.error({err: err}, 'fail to set clients');
        }
        if (callback) {
