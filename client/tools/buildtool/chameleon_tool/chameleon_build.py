...
def checkConflictedAssets(aapt, inputPkg, filelist):
    contents = runcmd([aapt, 'list', inputPkg])
...
def mergeAsset(aapt, targetPackage, parentFolder, filelist):
    ...
    try:
        ...
        p = os.path.relpath(targetPackage, parentFolder)
        buildlog("%s -> %s = %s" %(targetPackage, parentFolder, p))
...
def mergeDependLibAssets(ps, ig):
    assets = []
    for p in ps:
...
def mergeAssetsToApk(args):
    ...
    assets = mergeDependLibAssets(assets, ig)
    totalFileList = [x[0] for x in assets]
    assets = groupAssets(assets)
    if checkConflictedAssets(aapt, inputPkg, totalFileList):
        return -1
    else:
        buildlog(totalFileList)
        buildlog("copy file from %s to %s" %(inputPkg, targetpkg))
        shutil.copy2(inputPkg, targetpkg)
        for a in assets:
